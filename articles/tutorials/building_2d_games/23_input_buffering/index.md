...
1. Adding new fields to the `Slime` class to store the input buffer.
2. Updating how input is processed in the `HandleInput` method to account for the buffer.
3. Modify how movement is applied in the `Move` method.

Let's start by modifying the `Slime` class to include an input buffer. Open the *Slime.cs* file and add the following fields:
...
Next, we need to initialize this queue in the `Initialize` method:

[!code-csharp[](./snippets/slime/initialize.cs?highlight=30-31)]

### Updating the HandleInput Method

Next, we need to update the `HandleInput` method.  Instead of immediately overwriting our next direction, we will add valid input directions to the queue.  Update `HandleInput` to the following:
...
### Updating the Move Method

Finally, we need to modify how we apply the movement direction during the movement update cycle.  Make the following changes to the start of the `Move` method:

[!code-csharp[](./snippets/slime/move.cs?highlight=3-6)]
